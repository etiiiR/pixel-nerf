...
def extra_args(parser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Freeze encoder weights during initial warmup epochs",
    )
    parser.add_argument(
...
if args.freeze_enc and args.warmup_epochs > 0 and hasattr(net, 'encoder'):
    print(f"Freezing selected ResNet encoder layers for first {args.warmup_epochs} epochs")
    net.stop_encoder_grad = True
    net.encoder.eval()
    # Freeze layers up to 'layer2'
    for name, module in net.encoder.named_children():
...
class PixelNeRFTrainer(trainlib.Trainer):
    def __init__(self):
        ...
        if args.freeze_enc and self.warmup_epochs > 0:
            steps_per_epoch = math.ceil(len(dset) / args.batch_size)
    ...
    def calc_losses(self, data, is_train=True, global_step=0):
        ...
        print(f"global_step {global_step}, warmup_steps {self.warmup_steps}")
        print(f"encoder grad {net.stop_encoder_grad}")
        if global_step == self.warmup_steps and self.warmup_steps > 0:
            print("Warmup complete: unfreezing encoder")
            net.stop_encoder_grad = False
            net.encoder.train()
            for _, p in net.encoder.named_parameters():
        ...
        # rest of your calc_losses unchanged...
        all_bboxes = data.get("bbox") if self.use_bbox and is_train else None
